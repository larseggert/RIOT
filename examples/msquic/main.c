...
// the app (used for persistent storage and for debugging).
const QUIC_REGISTRATION_CONFIG RegConfig = {"quicriot",
                                            QUIC_EXECUTION_PROFILE_LOW_LATENCY};

// The protocol name used in the Application Layer Protocol Negotiation (ALPN).
const QUIC_BUFFER Alpn = {sizeof("riot") - 1, (uint8_t *)"riot"};
...
// The client's callback for stream events from MsQuic.

QUIC_STATUS QUIC_API ClientStreamCallback(HQUIC Stream,
                                          void * Context
...
void ClientSend(HQUIC Connection)
{
    ...
    if (QUIC_FAILED(
        ...
        printf("StreamOpen failed, 0x%" PRIx32 "!\n", Status);
        goto Error;
    }
    ...
    if (QUIC_FAILED(Status = MsQuic->StreamStart(
        ...
        printf("StreamStart failed, 0x%" PRIx32 "!\n", Status);
        MsQuic->StreamClose(Stream);
        goto Error;
    }
    ...
    if (QUIC_FAILED(Status =
        ...
        printf("StreamSend failed, 0x%" PRIx32 "!\n", Status);
        free(SendBufferRaw);
        goto Error;
    }
                                              ...
                                              QUIC_CONNECTION_EVENT * Event)
{
    ...
    case QUIC_CONNECTION_EVENT_SHUTDOWN_INITIATED_BY_TRANSPORT:
        ...
        if (Event->SHUTDOWN_INITIATED_BY_TRANSPORT.Status ==
            ...
            printf("[conn][%p] Shut down by transport, 0x%" PRIx32 "\n",
...
BOOLEAN ClientLoadConfiguration(BOOLEAN Unsecure)
{
    ...
    if (QUIC_FAILED(Status = MsQuic->ConfigurationOpen(
                        ...
                        NULL, &Configuration))) {
        printf("ConfigurationOpen failed, 0x%" PRIx32 "!\n", Status);
        return FALSE;
    }
    ...
    if (QUIC_FAILED(Status = MsQuic->ConfigurationLoadCredential(
        ...
        printf("ConfigurationLoadCredential failed, 0x%" PRIx32 "!\n", Status);
        return FALSE;
    }
...
void RunClient(void)
{
    ...
    if (QUIC_FAILED(Status = MsQuic->ConnectionOpen(Registration,
                                                    ...
                                                    NULL, &Connection))) {
        printf("ConnectionOpen failed, 0x%" PRIx32 "!\n", Status);
        goto Error;
    }
    ...
    if (QUIC_FAILED(Status = MsQuic->ConnectionStart(Connection, Configuration,
                                                     ...
                                                     Target, UdpPort))) {
        printf("ConnectionStart failed, 0x%" PRIx32 "!\n", Status);
        goto Error;
    }
...
int main(void)
{
    ...
    if (QUIC_FAILED(Status = MsQuicOpen2(&MsQuic))) {
        printf("MsQuicOpen2 failed, 0x%" PRIx32 "!\n", Status);
        goto Error;
    }
    ...
    if (QUIC_FAILED(Status =
        ...
        printf("RegistrationOpen failed, 0x%" PRIx32 "!\n", Status);
        goto Error;
    }
